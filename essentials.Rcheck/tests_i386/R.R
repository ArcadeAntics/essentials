p <- essentials::ArgumentParser(


    description = essentials::dedent(if (.Platform$OS.type == "windows") "
    ...
    epilogue = essentials::dedent(if (.Platform$OS.type == "windows") "
...
p$add.version(
    exit = essentials::dedent(sprintf("
        %s -- \"%s\"
...
p$add.argument("--vanilla", action = "store_true",
    help = essentials::dedent(if (.Platform$OS.type == "windows") "
